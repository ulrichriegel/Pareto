...
**add LayerVariance, Simulation etc, Hinweis auf truncated versions**
...
Matching the given data with piecewise Pareto is easy: 
...
This approach always works for three layers, but it often does not work if we have three or more layers. For instance, it does not work for the following example:
...
The Pareto package provides an improved matching approach, which works for arbitrary numbers of layers with consistent expected losses:
